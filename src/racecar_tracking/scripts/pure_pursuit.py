...
class PurePursuit:
    def __init__(self):
        ...
        self.HORIZON = 1.2	                                                        # 前视距离
        self.L = 0.498                                                              # 车辆轴距
        self.Vel_x = 1.2                                                            # x方向速度
        self.Vel_x_Old = 0
        self.vx_alpha = 1
        self.Vel_x_min = 0.2
        self.SlowDistance = 0.3	                                                    # 减速距离
        self.StopDistance = 0.1	                                                    # 停止距离
    ...
    def nav_rviz_goal(self):
        ...
        while not rospy.is_shutdown():
            ...
            if len(self.currentPath.poses) > 1 and pose_x:
                ...
                if self.states == 1 and (time.time() - time_calculate) > 1.8:           # 到达目标点
                    rospy.loginfo('reach rviz goal')
                    break
	...
	# 导航到达目标 
    def nav_goal_reach(self,x,y,th):
        rospy.logwarn("nav_goal_reach start")
    ...
    def calculateTwistCommand(self,pose_x,pose_y,pose_yaw):	
        try:
            ...
            alpha = math.atan2(target_y - pose_y, target_x - pose_x) - pose_yaw # get angle difference
            # print("alpha = ", alpha)
            final_x = self.currentPath.poses[-1].pose.position.x
            final_y = self.currentPath.poses[-1].pose.position.y
            final_dis = math.sqrt(math.pow(pose_x - final_x, 2) + math.pow(pose_y - final_y, 2))
            l = math.sqrt(math.pow(pose_x - target_x, 2) + math.pow(pose_y - target_y, 2))
            
            # print("lad = " + str(lad) + ", targetIndex = " + str(targetIndex) + ", l = " + str(l))
            
            if(final_dis > self.StopDistance):
                self.stop_num = 0
                theta = math.atan(2 * self.L * math.sin(alpha) / l) * 2
                vel_a = math.tan(theta)*self.Vel_x/self.L
                # print("theta = ", theta)
                # print('vel_a = ', vel_a)
                # vel_a = self.KF.kalman(vel_a)

                if(final_dis < self.SlowDistance):
                ...
                # get twist command
                twistCmd = Twist()
                # twistCmd.linear.x = vel_x / (1 + abs(theta))
                twistCmd.linear.x = vel_x
                twistCmd.angular.z = vel_a

                # print("theta = " + str(theta) + ", twistCmd.linear.x = " + str(twistCmd.linear.x) + ", twistCmd.angular.z = " + str(twistCmd.angular.z))
...
def yaml_read(yaml_path):
	...
	print("goal_dict = ", goal_dict)                # 目标 列表
	print("nav_goals", nav_goals)                   # 目标名称 列表


# 初始化变量
goal_dict = []
nav_goals = []
yaml_path = "/home/wgq/catkin_ws/src/racecar_tracking/config/goal.yaml"

# 定义发布目标函数，输入为地点名字符
def nav_reach(goal_name):
	if goal_name in nav_goals:
		print("尝试导航去:"+goal_name)
		goal_coordinate = goal_dict[goal_name]
		print("goal_data[0] : ", goal_coordinate[0])
		pure_pursuit_tracker.nav_goal_reach(goal_coordinate[0], goal_coordinate[1], goal_coordinate[2])

if __name__ == '__main__':
    try:
        pure_pursuit_tracker = PurePursuit()
        # yaml_read(yaml_path)

        # while not rospy.is_shutdown():
        #     pure_pursuit_tracker.nav_rviz_goal()
        #     nav_reach("end")
