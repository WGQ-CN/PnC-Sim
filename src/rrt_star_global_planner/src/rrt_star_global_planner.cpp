...
PLUGINLIB_EXPORT_CLASS(rrt_star_global_planner::RRTStarGlobalPlanner, nav_core::BaseGlobalPlanner)
...
namespace rrt_star_global_planner {
    ...
    RRTStarGlobalPlanner::RRTStarGlobalPlanner () {

    }

    RRTStarGlobalPlanner::RRTStarGlobalPlanner(std::string name, costmap_2d::Costmap2DROS* costmap_ros) {
        initialize(name, costmap_ros);
    }

    void RRTStarGlobalPlanner::initialize(std::string name, costmap_2d::Costmap2DROS* costmap_ros) {
        initialize(name, costmap_ros->getCostmap(), costmap_ros->getGlobalFrameID());
    }

    void RRTStarGlobalPlanner::initialize(std::string name, costmap_2d::Costmap2D* costmap, std::string frame_id) {
        if (!initialized_) {
    ...
    bool RRTStarGlobalPlanner::makePlan(const geometry_msgs::PoseStamped& start, const geometry_msgs::PoseStamped& goal, std::vector<geometry_msgs::PoseStamped>& plan) {
        ...
        for (int i = 0; i < max_iter_; ++i) {
            ...
            if (checkIfOutsidePlayArea(new_node) && isPathFree(nearest_node, new_node)) {
                chooseParent(nearest_node, new_node, node_list);
    ...
    void RRTStarGlobalPlanner::publishPlan(const std::vector<geometry_msgs::PoseStamped>& path) {
        if (!initialized_) {
            ROS_ERROR(
                    "This planner has not been initialized yet, but it is being used, please call initialize() before use");
            return;
    ...
    void RRTStarGlobalPlanner::clearRobotCell(const geometry_msgs::PoseStamped& global_pose, unsigned int mx, unsigned int my) {
        if (!initialized_) {
            ROS_ERROR(
                    "This planner has not been initialized yet, but it is being used, please call initialize() before use");
            return;
    ...
    void RRTStarGlobalPlanner::mapToWorld(double mx, double my, double& wx, double& wy) {
        wx = mx * costmap_->getResolution() + costmap_->getOriginX();
        wy = my * costmap_->getResolution() + costmap_->getOriginY();
    }
    
    double RRTStarGlobalPlanner::manhattanDistance(double x1, double y1, double x2, double y2) {
        double res = fabs(x2 - x1) + fabs(y1 - y2);
        return res;
    }

    double RRTStarGlobalPlanner::euclideanDistance(double x1, double y1, double x2, double y2) {
        double res = sqrt((x2 - x1)*(x2 - x1) + (y2 - y1)*(y2 - y1));
        return res;
    }

    NodePtr RRTStarGlobalPlanner::steer(const NodePtr& from_node, const NodePtr& to_node) {
        ...
        //     new_node->path_y_.push_back(new_node->y_);
        // }
        new_node->x_ = new_node->x_ + extend_length * cos(dist_and_theta.second);
        new_node->y_ = new_node->y_ + extend_length * sin(dist_and_theta.second);
    ...
    double RRTStarGlobalPlanner::calcDist2Goal(int x, int y, int goal_i) {
    ...
    NodePtr RRTStarGlobalPlanner::getRandomNode(int goal_i) {
    ...
    int RRTStarGlobalPlanner::getNearestNodeIndex(const std::vector<NodePtr>& node_list, const NodePtr& rnd_node) {
        int min_ind = 0;
    ...
    bool RRTStarGlobalPlanner::checkIfOutsidePlayArea(const NodePtr& node) {
    ...
    // bool RRTStarGlobalPlanner::checkCollision(const NodePtr& node) {
    ...
    std::pair<double, double> RRTStarGlobalPlanner::clacDistanceAndAngle(const NodePtr& from_node, const NodePtr& to_node) {
    ...
    bool RRTStarGlobalPlanner::isCellFree(int index) {
    ...
    bool RRTStarGlobalPlanner::isCellFree(int x, int y) {
