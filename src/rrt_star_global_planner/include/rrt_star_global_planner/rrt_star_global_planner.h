#ifndef _RRT_STAR_GLOBAL_PLANNER_H
#define _RRT_STAR_GLOBAL_PLANNER_H
...
class RRTStarGlobalPlanner : public nav_core::BaseGlobalPlanner {
    public:
        ...
        void chooseParent(NodePtr& nearest_node, NodePtr& new_node, const std::vector<NodePtr>& node_list);
        void rewire(NodePtr& new_node, std::vector<NodePtr>& node_list);
        bool isPathFree(const NodePtr& from_node, const NodePtr& to_node);
        bool isPathFree(int x1, int y1, int x2, int y2);

        void waypointOptimize(std::vector<geometry_msgs::PoseStamped>&, const std::vector<geometry_msgs::PoseStamped>&);
